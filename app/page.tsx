...
export default function Home() {
  ...
  const [reviewTab, setReviewTab] = useState<"week" | "month" | "year">("week");
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        <button className="brand" onClick={() => go("today")} aria-label="回到今天">
      ...
      <section className="workspace">
        ...
        {view === "trajectory" && (
          <TrajectoryView
            ...
            done={() => { setToast("周复盘已完成，下周重点已回到今天"); go("today"); }}
          />
        )}
      </section>
...
function TrajectoryView({ tab, setTab, decisions, setDecision, nextFocus, toggleNext, done }: {
  tab: "week" | "month" | "year"; setTab: (tab: "week" | "month" | "year") => void; decisions: Record<number, string>; setDecision: (id: number, value: string) => void;
  ...
  const options = [{ id: 1, title: "完成核心闭环原型", meta: "直接推进 · 建议 2 次专注" }, { id: 2, title: "完成 5 次用户访谈", meta: "验证问题 · 建议 3 次专注" }, { id: 3, title: "整理本月产品反馈", meta: "支持工作 · 建议 1 次专注" }];

  if (tab !== "week") return <div className="standard-page trajectory-page">
    <header className="trajectory-header"><div><span className="eyebrow">你的方向，不必一次想完</span><h1>轨迹</h1></div><div className="period-tabs"><button onClick={() => setTab("week")}>本周</button><button className={tab === "month" ? "active" : ""} onClick={() => setTab("month")}>本月</button><button className={tab === "year" ? "active" : ""} onClick={() => setTab("year")}>今年</button></div></header>
    <section className="long-horizon"><span>{tab === "month" ? "8 月 · 正在形成" : "2026 · 还在了解你"}</span><h2>{tab === "month" ? "把对产品的模糊想法，变成可验证的真实问题" : "正在显现的长期方向"}</h2><p>{tab === "month" ? "这是根据 3 次已确认周复盘形成的月度主题，不是从任务数量推算的完成率。" : "年度方向将在至少 3 个月的确认记录后提出。现在，系统只展示已发生的事实。"}</p>
      <div className="horizon-weeks">{["第 1 周\n问题发散", "第 2 周\n竞品研究", "第 3 周\n聚焦用户", "本周\n闭环成形"].map((item, index) => <article key={item} className={index === 3 ? "active" : ""}><i /><span>{item.split("\n")[0]}</span><b>{item.split("\n")[1]}</b></article>)}</div>
    </section>
  </div>;

  return <div className="standard-page trajectory-page">
    <header className="trajectory-header"><div><span className="eyebrow">8 月 10 日—16 日</span><h1>这一周，你真正推动了什么？</h1><p>事实已经整理好。你只需要确认它的意义。</p></div><div className="period-tabs"><button className="active" onClick={() => setTab("week")}>本周</button><button onClick={() => setTab("month")}>本月</button><button onClick={() => setTab("year")}>今年</button></div></header>
