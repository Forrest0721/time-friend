...
export default function DemoAppV2() {
  ...
  const [activeCollection, setActiveCollection] = useState<Collection>("today");
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("jianshi-prototype-tasks-v2");
  ...
  useEffect(() => { window.localStorage.setItem("jianshi-prototype-tasks-v2", JSON.stringify(tasks)); }, [tasks]);
...
function TaskWorkspace({ tasks, allTasks, groups, activeCollection, newTask, setNewTask, newTaskKind, setNewTaskKind, newTaskDue, setNewTaskDue, composerOpen, setComposerOpen, addTask, addSubtask, addGroup, deleteTask, toggleTask, startFocus, onTrajectory, onToggleSidebar, selectedTask, selectTask, updateTask, detailTab, setDetailTab, detailProgress, setDetailProgress, addDetailProgress, lists }: TaskWorkspaceProps) {
  ...
  return <div className={`dida-task-layout ${selectedTask ? "with-detail" : ""}`}>
    <section className="dida-task-pane">
      ...
      <form className={`dida-task-composer ${composerOpen ? "expanded" : ""}`} onSubmit={addTask}>
        <div><Plus /><input aria-label="快速添加任务" value={newTask} onFocus={() => setComposerOpen(true)} onChange={(event) => setNewTask(event.target.value)} placeholder="添加任务" /><button>添加</button></div>
        {composerOpen && <footer><div><button type="button" className={newTaskKind === "task" ? "active" : ""} onClick={() => setNewTaskKind("task")}><CheckSquare />任务</button><button type="button" className={newTaskKind === "note" ? "active" : ""} onClick={() => setNewTaskKind("note")}><FileText />笔记</button><label><CalendarDays /><input type="date" value={newTaskDue} onChange={(event) => setNewTaskDue(event.target.value)} /></label></div><button type="button" onClick={() => setComposerOpen(false)}><X />收起</button></footer>}
      </form>
      <div className="dida-task-list">
        {sections.map((section) => <section className="dida-task-group" key={section.id}><div className="dida-group-heading"><span>{section.name}</span><small>{section.items.length}</small><button aria-label={`更多${section.name}`} onClick={() => activeList && addGroup(activeList)}><MoreHorizontal /></button></div>{section.items.map((task) => <TaskRow key={task.id} task={task} selected={selectedTask?.id === task.id} toggleTask={toggleTask} selectTask={selectTask} startFocus={startFocus} deleteTask={deleteTask} />)}</section>)}
        ...
        <details className="dida-completed" open={activeCollection !== "today" && doneTasks.length > 0}><summary>已完成 <span>{doneTasks.length}</span></summary>{doneTasks.map((task) => <TaskRow key={task.id} task={task} selected={selectedTask?.id === task.id} toggleTask={toggleTask} selectTask={selectTask} startFocus={startFocus} deleteTask={deleteTask} />)}</details>
      </div>
    </section>
...
function TaskRow({ task, selected, toggleTask, selectTask, startFocus, deleteTask }: { task: Task; selected: boolean; toggleTask: (id: number) => void; selectTask: (id: number) => void; startFocus: (id: number) => void; deleteTask: (id: number) => void }) {
  ...
  return <article className={`dida-task-row ${selected ? "selected" : ""} ${task.done ? "done" : ""} ${task.kind === "note" ? "note" : ""}`}>
    ...
    <button className="dida-task-copy" onClick={() => selectTask(task.id)}><b>{task.title}</b><small><span className={`list-dot ${listColors[task.list] ?? "gray"}`} />{task.list}{task.steps.length ? ` · ${stepDone}/${task.steps.length}` : ""}{task.actual ? ` · ${task.actual} 分钟` : ""}</small></button>
...
function FocusWorkspace({ task, tasks, taskId, setTaskId, focusStage, setFocusStage, mode, switchMode, seconds, running, setRunning, reset, end, onManualRecord }: FocusWorkspaceProps) {
  ...
  return <div className={`focus-workspace-v2 ${focusStage ? "stage-open" : ""}`}>
    <section className="focus-main-v2">
      ...
      {!focusStage ? <div className="focus-project-list">{visibleFocusTasks.length === 0 && <p className="focus-empty">这里还没有任务。</p>}{visibleFocusTasks.map((item) => <article className={item.id === taskId ? "selected" : ""} key={item.id} onClick={() => setTaskId(item.id)}><span className={`focus-project-icon ${listColors[item.list] ?? "gray"}`}><ListChecks /></span><div><b>{item.title}</b><small>{item.list}</small></div><em>{item.actual}m</em>{!item.done && <button onClick={(event) => { event.stopPropagation(); setTaskId(item.id); setFocusStage(true); setRunning(true); }} aria-label={`开始专注${item.title}`}><Play /></button>}</article>)}</div> : <div className="focus-stage-v2">
      ...
      {!focusStage && <div className="focus-bottom-dock"><span className={`focus-project-icon ${listColors[task.list] ?? "gray"}`}><Timer /></span><div><small>专注</small><b>{task.title}</b></div><button className={mode === "countdown" ? "active" : ""} onClick={() => switchMode("countdown")}>25:00</button><button className={mode === "stopwatch" ? "active" : ""} onClick={() => switchMode("stopwatch")}>正计时</button><button className="dock-play" onClick={() => { setFocusStage(true); setRunning(true); }} aria-label="开始计时"><Play /></button></div>}
